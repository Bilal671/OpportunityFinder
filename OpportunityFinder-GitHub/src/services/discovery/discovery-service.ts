...
export class DiscoveryService {
  ...
  async startSearch(workspaceId: string, params: SearchParams): Promise<SearchRecord> {
    ...
    // On Vercel / serverless runtimes, await execution to prevent Lambda freezing before completion
    if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
      ...
      this.executePipeline(searchId, workspaceId, params).catch((err) => {
        console.error(`Error in search pipeline ${searchId}:`, err);
        store.updateSearch(searchId, {
          status: 'FAILED',
          errorMessage: err instanceof Error ? err.message : 'Search processing error',
  ...
  private async executePipeline(searchId: string, workspaceId: string, params: SearchParams) {
    ...
    const provider = providerRegistry.getProvider(params.provider || 'mock');
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      const progressBase = 25 + Math.round(((i + 1) / candidates.length) * 70);
      ...
      // STAGE 2-4: CRAWLING & AUDITING
      store.updateSearch(searchId, {
        stage: hasUrl ? 'CRAWLING' : 'SCORING',
