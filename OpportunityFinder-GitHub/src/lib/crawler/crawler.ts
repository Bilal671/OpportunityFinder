...
const REQUEST_TIMEOUT_MS = 4000; // 4s limit to ensure serverless functions remain responsive
...
export async function crawlWebsite(baseUrl: string): Promise<CrawlResult> {
  try {
    ...
    // Crawl up to 3 priority subpages
    for (const subUrl of priorityUrls.slice(0, 3)) {
      try {
        const subPage = await safeFetchPage(subUrl);
        pages.push(subPage);
      } catch {
        // Tolerant to subpage failure
      }
    }
